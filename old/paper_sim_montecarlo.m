...
for idx = 1:N    
    x_est_svd_radius(idx)       = 0.5*getMaxDist(x_est_svd_z{idx});
    x_est_opt_radius(idx)       = 0.5*getMaxDist(x_est_opt_z{idx});
        
    x_est_svd_con_radius(idx)   = 0.5*getMaxDist(x_est_svd_con_z{idx});
    x_est_opt_con_radius(idx)   = 0.5*getMaxDist(x_est_opt_con_z{idx});

%     [U,S,V] = svd(squeeze(P(:,:,idx)));
%     ra = S(1,1);
%     rb = S(2,2); 
%     x_n4sid_max_radius(idx) = norm([ra rb]);
end
...
hold on
grid on
plot(x_est_svd_radius,'k-');
plot(x_est_opt_radius,'r');
plot(x_est_svd_con_radius,'k--');
plot(x_est_opt_con_radius,'r--');
% plot(x_n4sid_max_radius,'b');
legend('Method 1','Method 2', 'Method 1 CON', 'Method 2 CON','Interpreter','latex');
ylim([-0.01 8.51]);
xlabel('$k$','Interpreter','latex');
ylabel('max. radius','Interpreter','latex');
