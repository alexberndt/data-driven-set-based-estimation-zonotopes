...
for i = 1:3
    ...
    % Sigma method assuming AV bound
    plot(z_zon_AV,[1 2],'m+-');
    z_zon_AV = M_Sigma*(z_zon_AV) + Z_w;
...
xlabel("state 1");
ylabel("state 2");
title({"Parameters",""," Noise bounds: c_w = " + c_w + ", c_{\gamma} = " + c_gam, "Extreme values?: " + useRandPointExtreme});
legend('Start','True','N4SID','Zono','Zono AV');
...
%     x_est_svd_con_kp1   = M_Sigma * (x_est_svd_con) + Z_w;
...
%     x_est_opt_con_kp1   = M_Sigma * (x_est_opt_con)  + Z_w;
...
gcf4 = figure(2);
set(gcf4, 'Position',  [100, 600, width, height])
clf;
% subplot(1,2,1);
bd = 25;
% xlim([-2.9 6.2]);
...
saveas(gcf4,'figures/sim_4.eps','epsc');
...
for idx = 4
    ...
    % METHOD 1
    % plot(x_est_svd_prev_z{idx},[1 2],'k--'); 
    plot(x_est_svd_z{idx},[1 2],'k*-');y^1
    plot(x_est_svd_con_z{idx},[1 2],'m*-');  
