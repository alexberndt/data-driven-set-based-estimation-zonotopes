...
h2  = semilogx(svd_cond_numbers, bounds.svd.sup, 'b+-');
h2b = semilogx(svd_cond_numbers, bounds.svd.inf, 'b+-');

h3  = semilogx(svd_cond_numbers, bounds.svdc.sup, 'r+-');
h3b = semilogx(svd_cond_numbers, bounds.svdc.inf, 'r+-');

h4  = semilogx(svd_cond_numbers, bounds.opt.sup, 'g+-');
h4b = semilogx(svd_cond_numbers, bounds.opt.inf, 'g+-');

h5  = semilogx(svd_cond_numbers, bounds.optc.sup, 'm+-');
h5b = semilogx(svd_cond_numbers, bounds.optc.inf, 'm+-');

xlim([0.01,6.5]);
ylim([-23, 15]);

xlabel("SVD condition number",'Interpreter','latex');
ylabel("bounds of $\mathcal{R}_{9}$ on $x_1$",'Interpreter','latex');
x0 = 10;
y0 = 10;
width = 450;
height = 220;
...
grid on
legend([h1,h2,h3,h4,h5],'True','Method 1','Method 1 CON','Method 2','Method 2 CON','Location','northeast','Interpreter','latex');
